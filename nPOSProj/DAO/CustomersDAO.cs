...
namespace nPOSProj.DAO
{
    class CustomersDAO
    {
        ...
        public String[,] ReadCustomer()
        {
            ...
            query += "FROM crm_customer ORDER BY crm_companyname";
            try
            {
                con.Open();
        ...
        public String[,] ReadCustomerSearch(String company)
        {
            Int32 count = this.PositionCountSearch(company);
            String[,] xxx = new String[7, count];
            con = new MySqlConnection();
            dbcon = new Conf.dbs();
            con.ConnectionString = dbcon.getConnectionString();
            ...
            query += "WHERE crm_companyname LIKE ?company ORDER BY crm_companyname";
            try
            {
                con.Open();
                MySqlCommand cmd = new MySqlCommand(query, con);
